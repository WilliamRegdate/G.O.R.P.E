...
int main()
{
    ...
    //create a player
    Player player(glm::vec3(0, 6, -30), width, height, world);

    //set up the level
	Model level("../resources/models/pit/model.gltf");
    ...
    for(int i = 0; i < 80; i++)
    {
        for(int j = 0; j < 2; j++)
        {
            balls.push_back(new Ball(glm::vec3(std::rand() % 10, 10 * i + 10, std::rand() % 10), world));
